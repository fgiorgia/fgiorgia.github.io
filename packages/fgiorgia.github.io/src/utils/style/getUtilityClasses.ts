...
const getClassName = (prefix: string | undefined, key: string, value: string | true): string => {
...
export const getUtilityClasses = (
  ...
  const utilityClasses = Object.entries(utilityProps)
    .reduce((classNames, [key, value]) => {
      ...
      if (styledClassName == null) {
        console.error(
          ...
          `Avaliable: ${Object.keys(styles).filter(k => !k.startsWith('__')).join(', ')}`
        )
        return classNames;
      }
